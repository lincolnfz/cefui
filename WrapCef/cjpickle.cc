...
namespace cyjh{
	...
	static const size_t cj_kCapacityReadOnly = static_cast<size_t>(-1);
	...
	inline void PickleIterator::Advance(size_t size) {
		size_t aligned_size = AlignInt(size, sizeof(unsigned int));
	...
	bool PickleIterator::ReadUInt32(unsigned int* result) {
		return ReadBuiltinType(result);
	}

	bool PickleIterator::ReadInt64(long long* result) {
		return ReadBuiltinType(result);
	}

	bool PickleIterator::ReadUInt64(unsigned long long* result) {
		return ReadBuiltinType(result);
	}
	...
	Pickle::Pickle(const char* data, int data_len)
		...
		header_size_(0),
		capacity_after_header_(cj_kCapacityReadOnly),
		write_offset_(0) {
	...
	Pickle::~Pickle() {
		if (capacity_after_header_ != cj_kCapacityReadOnly)
			free(header_);
	}
	...
	void Pickle::Reserve(size_t length) {
		size_t data_len = AlignInt(length, sizeof(unsigned int));
