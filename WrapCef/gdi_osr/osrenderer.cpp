...
void test32Bmp(const void* buffer, int width, int height){
	...
	if (hResult == S_OK && pStream)
	{
		...
		hResult = pStream->Write(&info, headlen, NULL);
		{
			...
			HANDLE hFile = CreateFileW(L"d:\\ssx.bmp", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_ARCHIVE, NULL);
			DWORD dw;
		...
		if (hResult == S_OK){
			...
			st = pBitmap->Save(L"d:\\aax.bmp", &bmpClsid, NULL);
		}
		pStream->Release();
	}
}
